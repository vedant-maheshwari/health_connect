// Enhanced AI Details HTML Template
// Replace lines 970-987 in patient_dashboard.html with this code

// AI Details Section - Rich Expandable Card
let aiDetailsHtml = '';
if (appointment.booking_source === 'ai') {
    try {
        const severityColor = getSeverityColor(appointment.severity);
        const severityClass = appointment.severity >= 4 ? 'high' : appointment.severity >= 3 ? 'medium' : 'low';
        const severityDesc = {
      ...
      <div id="ai-details-${appointment.id}" class="ai-details-expanded" style="display: none; background: #f8f9fa; padding: 16px; border-radius: 8px; margin-top: 8px; border-left: 4px solid #667eea;">
        ...
        ${appointment.symptoms ? `
          <div style="margin-bottom: 14px;">
            <strong style="color: #667eea; font-size: 0.9em;">💊 Symptoms Reported:</strong>
            <p style="margin: 6px 0; padding: 8px; background: white; border-radius: 6px; font-size: 0.9em; color: #555;">${appointment.symptoms}</p>
          </div>
        ` : ''}
