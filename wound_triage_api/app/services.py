...
llm = ChatOpenAI(model="gpt-4o", temperature=0, openai_api_key=settings.openai_api_key, request_timeout=60.0)
...
def get_relevant_lessons(complaint: str, limit: int = 3, translation: Optional[str] = None) -> List[Dict]:
    ...
    try:
        ...
        for lesson in all_lessons:
            try:
                lesson_emb = lesson.get("complaint_embedding")
                
                if not lesson_emb or len(lesson_emb) != len(query_embedding):
                ...
                # CRITICAL: Lowered threshold to 0.50 for cross-language matching
                # Cohere multilingual embeddings score lower for English ↔ Hindi/Hinglish:
                # - Same language: 0.70-0.90
                # - Cross-language: 0.50-0.65
                # LLM semantic filter (0.5 threshold) will prevent false positives
                if similarity < 0.50:
                    continue
        ...
        if not scored_lessons:
            print("❌ No relevant lessons found above similarity threshold (0.50)")
...
def analyze_triage(text: str, image_data: Optional[str] = None, language: str = "en", translation: Optional[str] = None) -> Triage:
    ...
    if image_data:
        # CRITICAL: Add explicit instruction to prevent GPT-4 refusal on medical images
        image_instruction = """
IMPORTANT IMAGE ANALYSIS INSTRUCTION:
You are a medical triage assistant helping route patients to appropriate specialists.
When an image is provided, describe what you observe visually (e.g., redness, swelling, wound type) and use that to inform your specialty recommendation.
You are NOT providing a diagnosis - you are triaging to the right specialist.
You MUST still return valid JSON in the exact format specified above.
If you cannot analyze the image, base your assessment on the text description only, but still return valid JSON.
"""
        messages[0] = SystemMessage(content=system_prompt + image_instruction)
        
        messages.append(
            HumanMessage(
                content=[
                    {"type": "text", "text": "Here is an image of the patient's condition. Describe what you observe and use it to inform your triage assessment:"},
                    {
                        "type": "image_url",
    ...
    try:
        ...
        # Check if GPT-4 refused to analyze (e.g., "I'm unable to...")
        if "unable" in content.lower() or "can't" in content.lower() or "cannot" in content.lower():
            if not content.strip().startswith("{"):
                print(f"⚠️  GPT-4 refused image analysis, retrying with text-only...")
                # Retry without image, using only text description
                messages_text_only = [
                    SystemMessage(content=system_prompt),
                    HumanMessage(content=f"Patient Complaint: {text}")
                ]
                response = llm.invoke(messages_text_only)
                content = response.content
